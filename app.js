...
// ─────────────────────────────────────────────────────────
// 문제 무한 생성기 — 매번 다른 숫자로 같은 개념의 새 문제 생성
// ─────────────────────────────────────────────────────────
...
const PROBLEM_GENERATORS = {
    'N01': () => {
        const a = _rand(3, 9), b = _rand(3, 9);
        const ans = a * b;
        return {
            난이도: '1', 유형: '계산', 용도: '진단',
            문제: `${a} × ${b} = ?`, 정답: String(ans), 해설: `${a}×${b}=${ans}`,
            오답1: String(ans - 1), 약점1: 'N01', 오답해설1: '구구단 정확히 외우기',
            오답2: String((a - 1) * b), 약점2: 'N01', 오답해설2: 'a 를 한 칸 잘못',
            오답3: String(a + b), 약점3: 'N01', 오답해설3: '곱셈을 덧셈으로',
            오답4: String(a * (b - 1)), 약점4: 'N01', 오답해설4: 'b 를 한 칸 잘못',
        };
    },

    'F03': () => {
        const dPairs = [[2,3],[3,4],[4,5],[2,5],[3,5],[3,7],[4,7],[5,6]];
        const [d1, d2] = _randChoice(dPairs);
        const L = _lcm(d1, d2);
        const m1 = L / d1, m2 = L / d2;
        const n1 = _rand(1, d1 - 1), n2 = _rand(1, d2 - 1);
        return {
            난이도: '1', 유형: '계산', 용도: '진단',
            문제: `${n1}/${d1} 과 ${n2}/${d2} 를 통분하시오`,
            정답: `${n1*m1}/${L} 과 ${n2*m2}/${L}`,
            해설: `최소공배수 ${L} 로 통분`,
            오답1: `${n1}/${d1}와 ${n2}/${d2}`, 약점1: 'F03', 오답해설1: '통분 시도 X',
            오답2: `${n1}/${L}와 ${n2}/${L}`, 약점2: 'F03', 오답해설2: '분자도 비례 변환 필요',
            오답3: `${n1*m2}/${L} 과 ${n2*m1}/${L}`, 약점3: 'F03', 오답해설3: '곱하는 수 거꾸로',
            오답4: `${n1}/${d1+d2}와 ${n2}/${d1+d2}`, 약점4: 'F03', 오답해설4: '분모를 더해버림',
        };
    },

    'F04': () => {
        const dPairs = [[2,3],[3,4],[2,5],[3,5],[4,5],[3,6],[2,7]];
        const [d1, d2] = _randChoice(dPairs);
        const L = _lcm(d1, d2);
        const n1 = _rand(1, d1 - 1), n2 = _rand(1, d2 - 1);
        const sumNum = n1 * (L/d1) + n2 * (L/d2);
        const g = _gcd(sumNum, L);
        const ansNum = sumNum / g, ansDen = L / g;
        return {
            난이도: '1', 유형: '계산', 용도: '진단',
            문제: `${n1}/${d1} + ${n2}/${d2} = ?`,
            정답: ansDen === 1 ? String(ansNum) : `${ansNum}/${ansDen}`,
            해설: `통분 후 분자 합 = ${sumNum}/${L}`,
            오답1: `${n1+n2}/${d1+d2}`, 약점1: 'F03', 오답해설1: '분자분모 따로 합',
            오답2: `${n1+n2}/${L}`, 약점2: 'F03', 오답해설2: '분자만 그대로 합',
            오답3: `${n1*n2}/${d1*d2}`, 약점3: 'F04', 오답해설3: '곱셈으로 처리',
            오답4: `${sumNum}/${L}`, 약점4: 'F02', 오답해설4: '약분 안 함',
        };
    },

    'F05': () => {
        const fracs = [[1,2],[2,3],[1,3],[3,4],[2,5],[3,5],[1,4]];
        const [n1, d1] = _randChoice(fracs);
        const [n2, d2] = _randChoice(fracs);
        ...
        return {
            ...
            정답: ansDen === 1 ? String(ansNum) : `${ansNum}/${ansDen}`,
            해설: `분자×분자 분모×분모 → 약분`,
            오답1: `${n1+n2}/${d1+d2}`, 약점1: 'F05', 오답해설1: '분자분모 따로 합',
            오답2: `${num}/${den}`, 약점2: 'F02', 오답해설2: '약분 안 함',
            오답3: `${n1*d2}/${d1*n2}`, 약점3: 'F05', 오답해설3: '교차 곱 (나눗셈으로 잘못)',
            오답4: `${n1}/${d2}`, 약점4: 'F05', 오답해설4: '한쪽만',
        };
    },

    'I02': () => {
        const a = _rand(-9, 9), b = _rand(-9, 9);
        if (a === 0 || b === 0) return PROBLEM_GENERATORS.I02();
        const ans = a + b;
        const aStr = a < 0 ? `(${a})` : String(a);
        const bStr = b < 0 ? `(${b})` : `+${b}`;
        return {
            난이도: '1', 유형: '계산', 용도: '진단',
            문제: `${aStr} ${bStr.startsWith('(') ? '+ ' + bStr : bStr} = ?`,
            정답: String(ans),
            해설: `같은 부호 → 절댓값 합 + 공통 부호. 다른 부호 → 절댓값 차 + 큰 쪽 부호`,
            오답1: String(Math.abs(a) + Math.abs(b)), 약점1: 'I01', 오답해설1: '부호 무시',
            오답2: String(-ans), 약점2: 'I02', 오답해설2: '부호 잘못',
            오답3: String(a - b), 약점3: 'I02', 오답해설3: '뺄셈으로',
            오답4: String(a * b), 약점4: 'I03', 오답해설4: '곱셈으로',
        };
    },

    'I03': () => {
        const a = _randChoice([-9,-8,-7,-6,-5,-4,-3,-2,2,3,4,5,6,7,8,9]);
        const b = _randChoice([-9,-8,-7,-6,-5,-4,-3,-2,2,3,4,5,6,7,8,9]);
        const ans = a * b;
        return {
            난이도: '1', 유형: '계산', 용도: '진단',
            문제: `(${a}) × (${b}) = ?`,
            정답: String(ans),
            해설: `음×음=양 음×양=음 양×양=양`,
            오답1: String(-ans), 약점1: 'I03', 오답해설1: '부호 규칙 미적용',
            오답2: String(a + b), 약점2: 'I02', 오답해설2: '곱셈을 덧셈으로',
            오답3: String(Math.abs(ans)), 약점3: 'I03', 오답해설3: '부호 빠뜨림',
            오답4: String(a - b), 약점4: 'I02', 오답해설4: '뺄셈으로',
        };
    },

    'M02': () => {
        const xVal = _rand(-5, 5);
        if (xVal === 0) return PROBLEM_GENERATORS.M02();
        const a = _rand(2, 6), b = _rand(-9, 9);
        const ans = a * xVal + b;
        const bStr = b >= 0 ? `+ ${b}` : `- ${-b}`;
        return {
            난이도: '2', 유형: '계산', 용도: '학습',
            문제: `x = ${xVal} 일 때 ${a}x ${bStr} 의 값은?`,
            정답: String(ans),
            해설: `${a}·${xVal} ${bStr} = ${ans}`,
            오답1: String(a + xVal + b), 약점1: 'M01', 오답해설1: 'ax 를 a+x 처럼',
            오답2: String(a * xVal), 약점2: 'M02', 오답해설2: '상수항 빠뜨림',
            오답3: String(-ans), 약점3: 'I02', 오답해설3: '부호 잘못',
            오답4: String(a * (xVal + 1) + b), 약점4: 'M02', 오답해설4: '대입 실수',
        };
    },

    'M03': () => {
        const a = _rand(2, 8), b = _rand(2, 7), c = _rand(1, 6);
        const ans = a + b - c;
        return {
            난이도: '1', 유형: '계산', 용도: '학습',
            문제: `${a}x + ${b}x - ${c}x 를 간단히 하시오`,
            정답: ans === 0 ? '0' : `${ans}x`,
            해설: `(${a}+${b}-${c})x = ${ans}x`,
            오답1: `${a + b}x`, 약점1: 'M03', 오답해설1: '-cx 빼먹음',
            오답2: `${a + b + c}x`, 약점2: 'I02', 오답해설2: '부호 처리 잘못',
            오답3: ans === 0 ? '0x' : String(ans), 약점3: 'M03', 오답해설3: '형식 잘못',
            오답4: `${a}x`, 약점4: 'M03', 오답해설4: '한 항만',
        };
    },

    'M05': () => {
        const a = _randChoice([-5,-4,-3,-2,2,3,4,5]);
        const b = _rand(2, 6), c = _randChoice([-5,-4,-3,-2,-1,1,2,3,4,5]);
        const ax = a * b, ac = a * c;
        const cStr = c >= 0 ? `+ ${c}` : `- ${-c}`;
        const aStr = a < 0 ? `(${a})` : String(a);
        const ansBStr = ax > 0 ? `${ax}x` : `${ax}x`;
        const ansCStr = ac >= 0 ? ` + ${ac}` : ` - ${-ac}`;
        return {
            난이도: '1', 유형: '전개', 용도: '학습',
            문제: `${aStr}(${b}x ${cStr}) 를 전개하시오`,
            정답: `${ansBStr}${ansCStr}`,
            해설: `분배법칙 ${a}·${b}=${ax} 그리고 ${a}·${c}=${ac}`,
            오답1: `${ansBStr}${c >= 0 ? ` + ${c}` : ` - ${-c}`}`, 약점1: 'M05', 오답해설1: '상수에 분배 X',
            오답2: `${a*b + ac}`, 약점2: 'M05', 오답해설2: '문자 빠뜨림',
            오답3: `${ax}x${ac > 0 ? ` - ${ac}` : ` + ${-ac}`}`, 약점3: 'I03', 오답해설3: '부호 잘못',
            오답4: `${a + b}x ${cStr}`, 약점4: 'M05', 오답해설4: '계수 합으로',
        };
    },

    'M06': () => {
        const a = _randChoice([-5,-4,-3,-2,-1,1,2,3,4,5]);
        const b = _randChoice([-5,-4,-3,-2,-1,1,2,3,4,5]);
        const sumAB = a + b, prodAB = a * b;
        const aStr = a >= 0 ? `+${a}` : `${a}`;
        const bStr = b >= 0 ? `+${b}` : `${b}`;
        const sumStr = sumAB > 0 ? `+ ${sumAB}x` : sumAB === 0 ? '' : ` - ${-sumAB}x`;
        const prodStr = prodAB > 0 ? ` + ${prodAB}` : prodAB === 0 ? '' : ` - ${-prodAB}`;
        return {
            난이도: '2', 유형: '전개', 용도: '진단',
            문제: `(x ${aStr})(x ${bStr}) 를 전개하시오`,
            정답: `x²${sumStr}${prodStr}`,
            해설: `x² + (a+b)x + ab`,
            오답1: `x²${prodStr}`, 약점1: 'M06', 오답해설1: '교차 곱 누락',
            오답2: `x² + ${a*b}x + ${sumAB}`, 약점2: 'P03', 오답해설2: '계수와 상수 거꾸로',
            오답3: `2x${sumStr}${prodStr}`, 약점3: 'M06', 오답해설3: 'x·x = x²',
            오답4: `x²${sumAB > 0 ? ` - ${sumAB}x` : sumAB === 0 ? '' : ` + ${-sumAB}x`}${prodStr}`, 약점4: 'I02', 오답해설4: '중간 항 부호',
        };
    },

    'P01': () => {
        const sign = _randChoice([1, -1]);
        const a = _rand(2, 7);
        const middle = sign * 2 * a;
        const last = a * a;
        const aStr = sign > 0 ? `+${a}` : `-${a}`;
        const midStr = middle > 0 ? ` + ${middle}x` : ` - ${-middle}x`;
        return {
            난이도: '2', 유형: '전개', 용도: '진단',
            문제: `(x ${aStr})² 을 전개하시오`,
            정답: `x²${midStr} + ${last}`,
            해설: `(a±b)² = a² ± 2ab + b²`,
            오답1: `x² + ${last}`, 약점1: 'P01', 오답해설1: '중간 항 빠뜨림',
            오답2: `x²${midStr.replace(' + ', ' - ').replace(' - ', ' + ')} + ${last}`, 약점2: 'P01', 오답해설2: '중간 항 부호',
            오답3: `x²${midStr} - ${last}`, 약점3: 'P01', 오답해설3: '끝 항 부호 (제곱은 양)',
            오답4: `x²${midStr.replace(/2/g, '1')} + ${last}`, 약점4: 'P01', 오답해설4: '중간 계수 2 빠짐',
        };
    },

    'P03': () => {
        const a = _randChoice([-6,-5,-4,-3,-2,-1,1,2,3,4,5,6]);
        const b = _randChoice([-6,-5,-4,-3,-2,-1,1,2,3,4,5,6]);
        if (a === b) return PROBLEM_GENERATORS.P03();
        const sumAB = a + b, prodAB = a * b;
        const aStr = a >= 0 ? `+${a}` : `${a}`;
        const bStr = b >= 0 ? `+${b}` : `${b}`;
        const sumStr = sumAB > 0 ? ` + ${sumAB}x` : sumAB === 0 ? '' : ` - ${-sumAB}x`;
        const prodStr = prodAB > 0 ? ` + ${prodAB}` : ` - ${-prodAB}`;
        return {
            난이도: '2', 유형: '전개', 용도: '학습',
            문제: `(x ${aStr})(x ${bStr}) 를 전개하시오`,
            정답: `x²${sumStr}${prodStr}`,
            해설: `x² + (a+b)x + ab`,
            오답1: `x²${prodStr}`, 약점1: 'P03', 오답해설1: '중간 항 빠뜨림',
            오답2: `x² + ${a*b}x + ${sumAB}`, 약점2: 'P03', 오답해설2: '계수·상수 거꾸로',
            오답3: `x²${sumStr.replace(' + ', ' XX ').replace(' - ', ' + ').replace(' XX ', ' - ')}${prodStr}`, 약점3: 'I02', 오답해설3: '중간 항 부호',
            오답4: `x²${sumStr}${prodAB > 0 ? ` - ${prodAB}` : ` + ${-prodAB}`}`, 약점4: 'I03', 오답해설4: '끝 항 부호',
        };
    },
    ...
    'P07': () => {
        const a = _randChoice([-6,-5,-4,-3,-2,-1,1,2,3,4,5,6]);
        const b = _randChoice([-6,-5,-4,-3,-2,-1,1,2,3,4,5,6]);
        if (a === b) return PROBLEM_GENERATORS.P07();
        const sumAB = a + b, prodAB = a * b;
        const sumStr = sumAB > 0 ? `+ ${sumAB}x` : sumAB === 0 ? '' : `- ${-sumAB}x`;
        const prodStr = prodAB > 0 ? `+ ${prodAB}` : `- ${-prodAB}`;
        const aStr = a >= 0 ? `+${a}` : `${a}`;
        ...
        return {
            난이도: '2', 유형: '인수분해', 용도: '진단',
            문제: `x² ${sumStr} ${prodStr} 를 인수분해하시오`,
            정답: `(x ${aStr})(x ${bStr})`,
            해설: `곱 ${prodAB} 합 ${sumAB} 인 두 수: ${a} 와 ${b}`,
            오답1: `(x ${a >= 0 ? '-' + a : '+' + (-a)})(x ${b >= 0 ? '-' + b : '+' + (-b)})`, 약점1: 'P07', 오답해설1: '두 수 부호 거꾸로',
            오답2: `(x ${aStr})(x ${a >= 0 ? '-' + a : '+' + (-a)})`, 약점2: 'P07', 오답해설2: '한 수 부호 잘못',
            오답3: `(x + ${prodAB})(x + 1)`, 약점3: 'P07', 오답해설3: '곱과 합 헷갈림',
            오답4: `(x + ${a + 1})(x + ${b - 1})`, 약점4: 'P07', 오답해설4: '두 수 어긋남',
        };
    },

    'E02': () => {
        const x = _rand(2, 9);
        const a = _rand(2, 5), b = _rand(1, 12);
        const c = a * x - b;
        return {
            난이도: '2', 유형: '방정식', 용도: '진단',
            문제: `${a}x - ${b} = ${c} 의 해는?`,
            정답: `x = ${x}`,
            해설: `${a}x = ${a*x} → x = ${x}`,
            오답1: `x = ${x + 1}`, 약점1: 'E02', 오답해설1: '계산 실수',
            오답2: `x = ${a * x}`, 약점2: 'E02', 오답해설2: `${a} 로 나누기 누락`,
            오답3: `x = ${-x}`, 약점3: 'I02', 오답해설3: '부호 잘못',
            오답4: `x = ${c - b}`, 약점4: 'E02', 오답해설4: '잘못된 이항',
        };
    },
    ...
    'E03': () => {
        const a = _randChoice([-5,-4,-3,-2,-1,1,2,3,4,5]);
        const b = _randChoice([-5,-4,-3,-2,-1,1,2,3,4,5]);
        if (a === b) return PROBLEM_GENERATORS.E03();
        const sumAB = a + b, prodAB = a * b;
        const sumStr = sumAB > 0 ? `+ ${sumAB}x` : sumAB === 0 ? '' : `- ${-sumAB}x`;
        const prodStr = prodAB > 0 ? `+ ${prodAB}` : `- ${-prodAB}`;
        // 해는 -a, -b
        const sols = [-a, -b].sort((x, y) => x - y);
        return {
            난이도: '2', 유형: '방정식', 용도: '진단',
            문제: `x² ${sumStr} ${prodStr} = 0 의 해는?`,
            정답: `x = ${sols[0]} 또는 x = ${sols[1]}`,
            해설: `(x+${a})(x+${b}) = 0 → x = ${-a} 또는 ${-b}`,
            오답1: `x = ${a} 또는 x = ${b}`, 약점1: 'E03', 오답해설1: '부호 잘못 (x+a=0 → x=-a)',
            오답2: `x = ${sols[0]}`, 약점2: 'E03', 오답해설2: '한 해만',
            오답3: `(x+${a})(x+${b})`, 약점3: 'E03', 오답해설3: '인수분해까지만',
            오답4: `x = ${-sumAB} 또는 x = ${-prodAB}`, 약점4: 'P07', 오답해설4: '잘못된 분해',
        };
    },

    'B02': () => {
        const x = _rand(-5, 8);
        const a = _rand(2, 5), b = _rand(-8, 8);
        const c = a * x + b;
        const ineq = _randChoice(['<', '>', '≤', '≥']);
        const flipMap = { '<': '>', '>': '<', '≤': '≥', '≥': '≤' };
        const ansIneq = ineq;  // a > 0 이라 그대로
        const bStr = b >= 0 ? `+ ${b}` : `- ${-b}`;
        return {
            난이도: '2', 유형: '부등식', 용도: '진단',
            문제: `${a}x ${bStr} ${ineq} ${c} 의 해는?`,
            정답: `x ${ansIneq} ${x}`,
            해설: `이항 후 ${a} 로 나눔 (양수 → 부등호 그대로)`,
            오답1: `x ${flipMap[ineq]} ${x}`, 약점1: 'B02', 오답해설1: '부등호 잘못 뒤집음',
            오답2: `x ${ansIneq} ${a * x}`, 약점2: 'B02', 오답해설2: `${a} 로 나누기 누락`,
            오답3: `x ${ansIneq} ${x + 1}`, 약점3: 'B02', 오답해설3: '계산 실수',
            오답4: `x ${ansIneq} ${-x}`, 약점4: 'I02', 오답해설4: '부호 잘못',
        };
    },

    'H02': () => {
        const a = _rand(2, 5), b = _rand(-9, 9);
        const xVal = _rand(-5, 5);
        if (xVal === 0) return PROBLEM_GENERATORS.H02();
        const ans = a * xVal * xVal + b;  // f(x) = ax² + b 형태
        const bStr = b >= 0 ? `+ ${b}` : `- ${-b}`;
        return {
            난이도: '2', 유형: '계산', 용도: '학습',
            문제: `f(x) = ${a}x² ${bStr} 일 때 f(${xVal}) 의 값은?`,
            정답: String(ans),
            해설: `${a}·${xVal}² ${bStr} = ${ans}`,
            오답1: String(a * xVal + b), 약점1: 'H02', 오답해설1: 'x² 처리 누락',
            오답2: String(-ans), 약점2: 'I03', 오답해설2: '음수 제곱 부호',
            오답3: String(a * xVal * xVal), 약점3: 'H02', 오답해설3: '상수항 누락',
            오답4: String(a + xVal * xVal + b), 약점4: 'H02', 오답해설4: '계수 처리',
        };
    },
};

function _generateProblem(conceptId) {
    const gen = PROBLEM_GENERATORS[conceptId];
    if (!gen) return null;
    try {
        const r = gen();
        return {
            '문제ID': `GEN_${conceptId}_${Date.now()}_${Math.floor(Math.random() * 100000)}`,
            '점검개념ID': conceptId,
            '난이도': r.난이도 || '2', '유형': r.유형 || '계산', '용도': r.용도 || '학습',
            '문제': r.문제 || '', '정답': r.정답 || '', '해설': r.해설 || '',
            '오답1': r.오답1 || '', '약점1': r.약점1 || conceptId, '오답해설1': r.오답해설1 || '',
            '오답2': r.오답2 || '', '약점2': r.약점2 || conceptId, '오답해설2': r.오답해설2 || '',
            '오답3': r.오답3 || '', '약점3': r.약점3 || conceptId, '오답해설3': r.오답해설3 || '',
            '오답4': r.오답4 || '', '약점4': r.약점4 || conceptId, '오답해설4': r.오답해설4 || '',
        };
    } catch (e) { console.warn('Generator failed for', conceptId, e); return null; }
}

function pickProblem(conceptId, opts = {}) {
    // 생성기가 있으면 매번 새 문제 생성 (외우기 방지)
    if (PROBLEM_GENERATORS[conceptId]) {
        const generated = _generateProblem(conceptId);
...
function startPractice(conceptId) {
    state.mode = 'practice';
    // 생성기 있으면 큐 비우고 매 문제마다 새로 만들기. 없으면 CSV 풀 사용.
    let queue = [];
    if (!PROBLEM_GENERATORS[conceptId]) {
        const pool = [...(state.problemsByConceptId[conceptId] || [])];
        pool.sort((a, b) => parseInt(a['난이도']) - parseInt(b['난이도']));
...
function nextPracticeQuestion() {
    ...
    // 생성기 있으면 매번 새 문제 생성. 없으면 큐에서 꺼냄.
    let next;
    if (PROBLEM_GENERATORS[state.practice.conceptId]) {
        next = _generateProblem(state.practice.conceptId);
    } else {
        next = state.practice.queue.shift() || null;
    }
